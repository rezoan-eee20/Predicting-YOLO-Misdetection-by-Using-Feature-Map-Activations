...
#Paper 1
def make_simple_model_paper(input_shape=(13, 13, 6)) -> keras.models.Model:
    inputs = keras.layers.Input(shape=input_shape, name='main_input')
    x = keras.layers.Conv2D(64, (3, 3), padding="same",
                            activation='relu')(inputs)
    x = keras.layers.MaxPool2D((3, 3), padding="same")(x)
...
def test_trained_model_nn_new(model, class_name, method, op_dir, x_val, y_val, meta_val):
    # data_path = '/media/bijay/Projects/Datasets/val/person/outputs/data.dat'
    # x_val, y_val, meta_val = pickle_load_save.load(data_path)
...
def train():
    ...
    tensorboard = keras.callbacks.TensorBoard(
        log_dir='logs/new_dt_'+model_name+'_'+class_name + num+'{}'.format(datetime.now().strftime("%Y%m%d-%H%M%S")), histogram_freq=3, write_grads=True)
    learning_rate_dec = tf.keras.callbacks.ReduceLROnPlateau(
        verbose=1, min_lr=0.0001, patience=3, factor=0.4)
    early_stopping = tf.keras.callbacks.EarlyStopping(patience=50, verbose=1)

    callbacks_list = [learning_rate_dec,
                      tensorboard, early_stopping]

    model = make_simple_model_paper(input_shape=(13, 13, 6))
    ...
    test_trained_model_nn(model, model_name, x_val, y_val)
...
if __name__ == "__main__":
    ...
    # test_data()
    train()
    # # # # method = ['sum', 'secondary', 'all', 'run_all'][1]
    # # # # runrfe = True

    # # # # for i in range(3):
        # # # # if i==0 and (method=='sum' or method=='run_all'):
            # # # # if runrfe:
              # # # # ''' RFE with sum '''
            ...
            # # # # else:
              ...
              # # # # n_features = len(final_selection)
            # # # # train_bn(final_selection, class_name, method, step, K, n_features, rfe_time)
        # # # # elif i==1 and (method=='secondary' or method=='run_all'):
            # # # # if runrfe:
            ...
            # # # # else:
              ...
              # # # # n_features = len(final_selection)
            # # # # train_bn(final_selection, class_name, method, step, K, n_features, rfe_time)
        # # # # elif i==2 and (method=='all' or method=='run_all'):
            # # # # ''' All Features '''
    ...
    # train_two_neuron()
     ...
     # test_trained_model(model=load_model_from_file(
           # checkpoint_name='./checkpoints/ndt_person_cnn_bn.h5'))

    # op_dir = './data/model_results/cnn_net1'
    # op_name = 'acc_6'
    # cm = np.array([[2702, 650], [712, 2640]])
    # plot_confusion_matrix_new(cm,  target_names=['False', 'True'], normalize=False,
                              # title="Confusion Matrix (th={:.2f})".format(0.5), path=op_dir, name=op_name+"_cm_acc")

    # op_name = 'acc_1_6'
    # cm = np.array([[2865, 487], [779, 2573]])
    # plot_confusion_matrix_new(cm,  target_names=['False', 'True'], normalize=False,
                              # title="Confusion Matrix (th={:.2f})".format(0.5), path=op_dir, name=op_name+"_cm_acc")

    # op_name = 'fpr_1_6'
    # cm = np.array([[1568, 1784], [166, 3184]])
    # plot_confusion_matrix_new(cm,  target_names=['False', 'True'], normalize=False,
                              # title="Confusion Matrix (th={:.2f})".format(0.27), path=op_dir, name=op_name+"_cm_acc")

    # exit(0)
    # op_name = 'acc_6'
    # cm = np.array([[2415, 937], [786, 2566]])
    # plot_confusion_matrix_new(cm,  target_names=['False', 'True'], normalize=False,
                              # title="Confusion Matrix (th={:.2f})".format(0.5), path=op_dir, name=op_name+"_cm_acc")

    # op_name = 'acc_3'
    # cm = np.array([[2333, 1019], [751, 2601]])
    # plot_confusion_matrix_new(cm,  target_names=['False', 'True'], normalize=False,
                              # title="Confusion Matrix (th={:.2f})".format(0.5), path=op_dir, name=op_name+"_cm_acc")

    # cm = np.array([[279, 51], [42, 288]])
